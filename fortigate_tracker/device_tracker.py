...
from homeassistant.const import CONF_HOST, CONF_PASSWORD, CONF_USERNAME, CONF_PORT, CONF_TIMEOUT
...
class FortigateAPI:
    """Handle Fortigate API connection and data."""

    def __init__(self, host, port, username, password, timeout=60):
        """Fortigate API class init."""
        self._host = host
        self._port = port
    ...
    def get_devices(self):
        ...
        if self._session is not None:
            ...
            if request.status_code == 200:
                ...
                _LOGGER.error("Could'n get devices from Fortigate, request status code %s",
                              request.status_code)
                return None
        else:
            return None
...
class FortigateDeviceTracker(DeviceScanner):
    """Fortigate Device Tracker Class."""

    def __init__(self, config):
        """Fortigate class init."""
    ...
    def _update_info(self):
        """Update data."""
        self.last_results = []
        fortigate = FortigateAPI(self._host,
                                 self._port,
                                 self._username,
                                 self._password,
                                 self._timeout)
            ...
            for device in devices:
                ...
                if int(device["last_seen"]) < self._timeout:
                    if device["mac"] is None:
                        ...
                        continue
                    _LOGGER.debug("Add device %s - %s - last seen: %s",
                                  device["mac"],
                                  name,
                                  device["last_seen"])
                    self.last_results.append(Device(device["mac"].upper(),
                                                    name,
                                                    ipaddr,
                                                    device["last_seen"]))
                else:
                    _LOGGER.debug("Ignoring device %s - %s - last seen: %s",
                                  device["mac"],
                                  name,
                                  device["last_seen"])
            del fortigate
