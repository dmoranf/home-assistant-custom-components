...
class FortigateAPI:
    def __init__(self,host,port,username,password,timeout=180):
        self._host = host
        self._port = port
...
class FortigateDeviceTracker(DeviceScanner):
    ...
    def scan_devices(self):
        self._update_info()
        return [device.mac for device in self._last_results]

    def get_device_name(self,device):
        name = [result.name for result in self._last_results if result.mac == device]
        return name[0]

    def get_extra_attributes(self,device):
        filter_ip = next (( result.ip for result in self._last_results if result.mac == device), None)
        return {'ip': filter_ip }
